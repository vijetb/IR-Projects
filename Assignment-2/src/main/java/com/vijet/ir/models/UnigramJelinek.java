...
public final class UnigramJelinek {
	...
	public static double score(TokenDesc token, Long lengthDoc, String[] queryTerms) {
		double score = 0.0;
		for(int i = 1; i < queryTerms.length; i++){
			//String qterm = StemUtil.getStemOfWord(queryTerms[i].trim().toLowerCase());
			String qterm = queryTerms[i].trim().toLowerCase();
			if(token.getTerm().equals(qterm)){
				score = score + computeScore(qterm, Long.valueOf(token.getCount()), lengthDoc);
			}else{
				score = score + computeScoreWithoutQueryTerm(qterm,Long.valueOf(token.getCount()),lengthDoc);
			}
		}
		return score;
	}
	...
	public static double score(TokenDesc token, Long docLength, Long total_freq_count) {
